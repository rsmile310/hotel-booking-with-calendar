Scheduler.plugin(function (e) {
  !(function () {
    ;(window.dhtmlXTooltip = e.dhtmlXTooltip = e.tooltip = {}),
      ...
      (dhtmlXTooltip.show = function (t, a) {
        if (!this._mobile || e.config.touch_tooltip) {
          ...
          if (!this.isTooltip(_)) {
            ...
            ;(r.visibility = 'hidden'),
              r.removeAttribute
                ? (r.removeAttribute('right'), r.removeAttribute('bottom'))
                : (r.removeProperty('right'), r.removeProperty('bottom')),
              (r.left = '0'),
              (r.top = '0'),
              e.config.rtl && (i.className += ' dhtmlXTooltip_rtl'),
              (this.tooltip.innerHTML = a),
              document.body.appendChild(this.tooltip)
            var l = this.tooltip.offsetWidth,
              c = this.tooltip.offsetHeight
            document.documentElement.clientWidth - d - l < 0
              ? (r.removeAttribute
                  ? r.removeAttribute('left')
                  : r.removeProperty('left'),
                (r.right =
                  document.documentElement.clientWidth -
                  d +
                  2 * (n.config.delta_x || 0) +
                  'px'))
              : (r.left =
                  d < 0
                    ? o.x + Math.abs(n.config.delta_x || 0) + 'px'
                    : d + 'px'),
              document.documentElement.clientHeight - s - c < 0
                ? (r.removeAttribute
                    ? r.removeAttribute('top')
                    : r.removeProperty('top'),
                  (r.bottom =
                    document.documentElement.clientHeight -
                    s -
                    2 * (n.config.delta_y || 0) +
                    'px'))
                : (r.top =
                    s < 0
                      ? o.y + Math.abs(n.config.delta_y || 0) + 'px'
                      : s + 'px'),
              ...
              (this.tooltip.onmouseleave = function (t) {
                t = t || window.event
                for (
                  var a = e.dhtmlXTooltip, n = t.relatedTarget;
                  n != e._obj && n;
              ...
              e.callEvent('onTooltipDisplayed', [
                this.tooltip,
                this.tooltip.event_id,
              ])
          }
        }
      }),
      (dhtmlXTooltip._clearTimeout = function () {
        this.tooltip._timeout_id &&
      ...
      (dhtmlXTooltip.position = function (e) {
        return (e = e || window.event), { x: e.clientX, y: e.clientY }
      }),
      e.attachEvent('onMouseMove', function (t, a) {
        ...
        if ((t && e.getState().editor_id != t) || o || _) {
          ...
          if (t || r.tooltip.event_id) {
            ...
            if (!s) return
            if (
              ((r.tooltip.event_id = s.id),
              !(d = e.templates.tooltip_text(s.start_date, s.end_date, s)))
            )
              return r.hide()
          }
          _ && (d = '')
